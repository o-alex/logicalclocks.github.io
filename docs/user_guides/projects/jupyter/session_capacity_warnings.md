...
| Instance | `This instance is full` | Red | The Hopsworks instance you would be routed to has no free session slots. `Run Jupyter`, `Start Terminal`, and `Start App` are disabled. |
...
On a single-instance deployment the two badges always agree and you only see one.
